...
from training.loss_utils import multitask_loss, multitask_loss_balance
...
# Training
def train(net, epoch, criterion, w_p, w_n):
    ...
    for batch_idx, bt in enumerate(trainloader):
        ...
        # labels = [targets[0].repeat(len(bt), 1), targets[1].repeat(len(bt), 1)]

        outputs = torch.cat((outputs_l, outputs_s))
        loss = criterion(outputs, targets, w_p, w_n)
        output_list = outputs.split(batch_size)
        ...
        for i in range(4):
            for j in range(i + 1, 4):
                loss_ot = F.mse_loss(output_list[i], output_list[j])
                losses += loss_ot
...
t_net, ids, _ = get_model(args.conv, frm='official', name_s=args.name_s, name_t=args.name_t, logger=logger,
                          ...
                          plug_in=args.plug_in, classifier=args.classifier, dropout=args.dropout, fc1=args.fc1, fc2=args.fc2)
if device == 'cuda':
...
criterion = multitask_loss_balance

for epoch in range(1, args.n_epochs+1):
    train(t_net, epoch, criterion, w_p, w_n)
